...
use crate::{
    ...
    decompiler::{
        ast::visitors::emit_context::EmitContext,
        function_decompiler::{FunctionDecompilerBuilder, FunctionDecompilerError},
    },
...
impl Module {
    ...
    ///
    /// # Arguments
    /// - `ctx`: The EmitContext to use for decompilation.
    ///
    /// # Returns
    /// - A Result containing the decompiled module if successful, or an error if not.
    pub fn decompile(&self, ctx: EmitContext) -> Result<String, FunctionDecompilerError> {
        let mut decompiled = String::new();
        for function in self.functions.iter() {
            let mut decompiler = FunctionDecompilerBuilder::new(function).build();
            let decompiled_function = decompiler.decompile(ctx)?;
            decompiled.push_str(&decompiled_function);
            decompiled.push('\n');
        }
        Ok(decompiled)
    }
