...
impl std::fmt::Display for OperandError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            OperandError::InvalidConversion(from, to) => {
                write!(f, "Attempted to convert {} operand to a {}", from, to)
            }
            OperandError::InvalidJumpTarget(addr) => write!(f, "Invalid jump target: {}", addr),
        }
    }
}
