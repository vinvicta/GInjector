...
impl Display for FunctionError {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match self {
            FunctionError::BasicBlockNotFoundById(id) => {
                write!(f, "BasicBlock not found by its block id: {:?}", id)
            }
            FunctionError::BasicBlockNotFoundByAddress(addr) => {
                write!(f, "BasicBlock not found by its address: 0x{:X}", addr)
            }
            FunctionError::BasicBlockNodeIndexNotFound(id) => {
                write!(f, "BasicBlock with id {:?} does not have a NodeIndex", id)
            }
            FunctionError::EntryBlockAlreadyExists => {
                write!(f, "Function already has an entry block")
            }
        }
    }
}
...
impl FunctionId {
    ...
    /// # Example
    /// ```
    /// use gs2_decompiler::function::FunctionId;
    ///
...
impl Function {
    ...
    /// # Example
    /// ```
    /// use gs2_decompiler::function::{Function, FunctionId};
    /// use gs2_decompiler::basic_block::BasicBlockType;
    ///
