...
impl std::fmt::Display for GraalIoError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            GraalIoError::NoNullTerminator() => {
                write!(f, "No null terminator found when reading a string.")
            }
            GraalIoError::ValueExceedsMaximum(value, max) => write!(
                f,
                "Value exceeds maximum for Graal-encoded integer. Value was {}, but cannot exceed {}.",
                value, max
            ),
            GraalIoError::Io(err) => write!(f, "IO error: {}", err),
        }
    }
}

impl std::error::Error for GraalIoError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            GraalIoError::Io(err) => Some(err),
            _ => None,
        }
    }
}

impl From<io::Error> for GraalIoError {
...
impl<R: Read> GraalReader<R> {
    ...
    /// # Examples
    /// ```
    /// use gs2_decompiler::graal_io::GraalReader;
    /// use std::io::Cursor;
    ///
...
impl<W: Write> GraalWriter<W> {
    ...
    /// # Examples
    /// ```
    /// use gs2_decompiler::graal_io::GraalWriter;
    /// use std::io::Cursor;
    ///
    /// let data = vec![0];
