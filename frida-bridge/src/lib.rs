...
pub struct InjectionOffsets {
    ...
    /// Byte pattern for TGraalVar constructor
    pub constructor_pattern: Option<String>,
    /// Byte pattern for TGraalVar::SetScript
...
impl FridaInjector {
    ...
    /// Check if using pattern scanning
    fn get_use_pattern_scanning(&self) -> bool {
        self.custom_offsets
            .as_ref()
    ...
    fn get_constructor_pattern(&self) -> Option<String> {
        self.custom_offsets
            .as_ref()
    ...
    fn get_setscript_pattern(&self) -> Option<String> {
        self.custom_offsets
            .as_ref()
    ...
    /// modeled after the logic in graalv6_gs2.js and worldsclient_gs2.js
        ...
        let use_pattern_scanning = self.get_use_pattern_scanning();
        let constructor_pattern = self.get_constructor_pattern();
        let setscript_pattern = self.get_setscript_pattern();
        ...
        // Build script header
        let mut script = format!("// Generated by GInjector - GS2 Bytecode Injector for {}\n", self.client_type.name());

        if use_pattern_scanning {
            script.push_str("// Using pattern scanning to find function addresses\n\n");
        } else {
            let constructor_offset = format!("0x{:X}", self.get_constructor_offset());
            let setscript_offset = format!("0x{:X}", self.get_setscript_offset());
            script.push_str(&format!("// Constructor offset: {}\n", constructor_offset));
            script.push_str(&format!("// SetScript offset: {}\n\n", setscript_offset));
        }
        ...
        if use_pattern_scanning {
            if let (Some(ctor_pat), Some(set_pat)) = (&constructor_pattern, &setscript_pattern) {
                script.push_str("    // Scan for TGraalVar constructor pattern\n");
                script.push_str(&format!("    var ctorPattern = \"{}\";\n", ctor_pat));
                script.push_str(&format!("    var ctorResults = Memory.scanSync(baseAddress, m.size, ctorPattern);\n"));
                script.push_str(&format!("    if (ctorResults.length <= {}) {{\n", pattern_index));
                script.push_str("        throw new Error(\"TGraalVar constructor pattern not found! Found \" + ctorResults.length + \" matches.\");\n");
                script.push_str("    }\n");
                script.push_str(&format!("    var ctorOffset = ctorResults[{}].address.sub(baseAddress);\n", pattern_index));
                script.push_str("    console.log(\"Found TGraalVar constructor at: \" + ctorOffset);\n\n");

                script.push_str("    // Scan for TGraalVar::SetScript pattern\n");
                script.push_str(&format!("    var setPattern = \"{}\";\n", set_pat));
                script.push_str(&format!("    var setResults = Memory.scanSync(baseAddress, m.size, setPattern);\n"));
                script.push_str(&format!("    if (setResults.length <= {}) {{\n", pattern_index));
                script.push_str("        throw new Error(\"TGraalVar::SetScript pattern not found! Found \" + setResults.length + \" matches.\");\n");
                script.push_str("    }\n");
                script.push_str(&format!("    var setOffset = setResults[{}].address.sub(baseAddress);\n", pattern_index));
                script.push_str("    console.log(\"Found TGraalVar::SetScript at: \" + setOffset);\n\n");
            } else {
                let constructor_offset = format!("0x{:X}", self.get_constructor_offset());
                let setscript_offset = format!("0x{:X}", self.get_setscript_offset());
                script.push_str(&format!("    var ctorOffset = ptr(\"{}\");\n", constructor_offset));
                script.push_str(&format!("    var setOffset = ptr(\"{}\");\n\n", setscript_offset));
            }
        } else {
            let constructor_offset = format!("0x{:X}", self.get_constructor_offset());
            let setscript_offset = format!("0x{:X}", self.get_setscript_offset());
        ...
        if let (Some(offset), Some(value)) = (magic_check_offset, magic_check_value) {
            script.push_str("    // Wait for client initialization\n");
            script.push_str(&format!("    var checkAddress = baseAddress.add(0x{:X});\n", offset));
            script.push_str(&format!("    while (checkAddress.readInt() !== {}) {{\n", value));
            script.push_str("        console.log(\"Waiting for client initialization...\");\n");
            script.push_str("        await sleep(100);\n");
            script.push_str("    }\n");
            script.push_str("    console.log(\"Client initialized!\");\n\n");
        }

        // Create NativeFunctions
        script.push_str("    const TGraalVar_TGraalVar = new NativeFunction(baseAddress.add(ctorOffset)");
        if uses_thiscall {
            script.push_str(", 'void', ['pointer', 'pointer'], 'thiscall');\n");
        } else {
            script.push_str(", 'void', ['pointer', 'pointer']);\n");
        }
        script.push_str("    const TGraalVar_SetScript = new NativeFunction(baseAddress.add(setOffset)");
        if uses_thiscall {
            script.push_str(", 'void', ['pointer', 'pointer'], 'thiscall');\n\n");
        } else {
            script.push_str(", 'void', ['pointer', 'pointer']);\n\n");
        ...
        script.push_str("    console.log(\"Waiting 5 seconds before injecting script...\");\n");
        script.push_str("    await sleep(5000);\n");
        ...
        script.push_str("}\n\n");
        script.push_str("init().catch(e => console.error(\"Injection error: \" + e));\n");
