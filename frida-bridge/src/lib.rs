...
impl FridaInjector {
    ...
    pub fn generate_injection_script(
        ...
        let _thiscall_suffix = if self.client_type.uses_thiscall() { ", 'thiscall'" } else { "" };

        let _magic_check = if let (Some(offset), Some(value)) = (
        ...
        if let (Some(offset), Some(_value)) = (self.client_type.magic_check_offset(), self.client_type.magic_check_value()) {
            script.push_str(&format!("{:X});\n", offset));
            script.push_str("    while (check_address.readInt() !== 157876074) {\n");
        ...
        if self.client_type.uses_thiscall() {
